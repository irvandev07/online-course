...
ins_course = db.Table('ins_course',
...
class Instructors(db.Model):
	...
	instructors = db.relationship('Course', backref='instructors', secondary='ins_course')
...
class Course(db.Model):
	...
	courses = db.relationship('Enroll', backref='course')
	courses_ins = db.relationship('Instructors', backref='courses', secondary='ins_course')

	def __repr__(self):
		return f'<Book "{self.name_course}">'
...
def create_course():
	...
	if not user:
		...
		cou.instructors.append(ins)
		db.session.add(cou)
		db.session.commit()
		# db.session.add(cou)
		# db.session.commit()
		# a= Course.query.all()
		# ins_ = Instructors.query.filter_by(id=ins.id).first_or_404()
		# course = Course.query.filter_by(id=a[-1].id).first_or_404()
		# ins_.instructors.append(course)
		# db.session.add(ins)
		# db.session.commit()
		# return {
		# 	'message' : 'success'
		...
		return jsonify([
			{
				...
				'name course': enroll.course.name_course,
